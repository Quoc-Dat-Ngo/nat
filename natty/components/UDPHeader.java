...
public class UDPHeader {
    /**
     * 
     * Constructs and sets the UDP header fields in the given packet.
     * 
     * 
     * @param packet    The full packet buffer (IP header + UDP header + data)
     * @param srcPort   Source port number
     * @param dstPort   Destination port number
     * @param dataBytes Payload data
     */
    public static void setUDPHeader(byte[] packet, int srcPort, int dstPort, byte[] dataBytes) {
        ...
        byte[] pseudoHeaderFields = { 127, 0, 0, 1, 127, 0, 0, 1, 0, 17, packet[24], packet[25] }; // Need to
                                                                                                   // reconstruct
                                                                                                   // hard-coded srcIP
                                                                                                   // and
                                                                                                   // dstIP
    ...
    /**
     * 
     * Computes the UDP checksum using 16-bit one's complement addition.
     * 
     * 
     * @param udpPacket The buffer containing pseudo-header + UDP header + data
     * @return The accumulated sum (before one's complement)
     */
    public static long computeUDPChecksum(byte[] udpPacket) {
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            if (i + 1 < length) {
                ...
                // Odd length case (last byte padded)
                word = (udpPacket[i] & 0xFF) << 8;
            }

            sum += word;

            // Wrap around carry (keep sum within 16 bits)
            if ((sum & 0xFFFF0000) != 0) {
                sum = (sum & 0xFFFF) + (sum >> 16);
            }
        }

        // Final wrap-around in case of remaining carry
        while ((sum >> 16) != 0) {
            sum = (sum & 0xFFFF) + (sum >> 16);
        }
